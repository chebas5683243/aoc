...
function processLineByLine() {
    ...
    return __awaiter(this, void 0, void 0, function () {
        var fileStream, rl, contents, _d, rl_1, rl_1_1, line, e_1_1;
...
function getDestinations(sources, map) {
    map.sort(function (a, b) { return b.source - a.source; });
    var destinations = sources.map(function (currSource) {
        for (var i = 0; i < map.length; i++) {
            ...
            if (combination.source <= currSource) {
                ...
                if (combination.source + combination.rangeLength - 1 >= currSource)
                    return combination.dest + currSource - combination.source;
                else
                    return currSource;
            }
        }
...
function getLocations(seeds, contents) {
    var currentSources = seeds;
    var map = [];
...
function getLowestLocation(contents) {
    var firstLine = contents[0];
    var _a = firstLine.split(":"), _ = _a[0], seedsString = _a[1];
    var seedsRanges = seedsString.trim().split(" ").map(function (str) { return Number(str); });
    var lowestLocation = Infinity;
    var nOperations = 0;
    for (var i = 0; i < seedsRanges.length; i += 2) {
        var initialValue = seedsRanges[i];
        var rangeLength = seedsRanges[i + 1];
        for (var j = 0; j < rangeLength; j++) {
            var seeds = [initialValue + j];
            var locations = getLocations(seeds, contents.slice(3));
            nOperations++;
            if (nOperations % 1000 === 0)
                console.log(nOperations);
...
function main() {
    return __awaiter(this, void 0, void 0, function () {
        var contents, lowestLocation;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, processLineByLine()];
                case 1:
                    contents = _a.sent();
                    lowestLocation = getLowestLocation(contents);
