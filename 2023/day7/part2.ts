...
function getTypeFromHand(handStr: string, bid: number) {
  ...
  const nJokers = repetitions[11] || 0;
  let maxReps = 0;

  let type = Object.keys(repetitions).reduce((acc, curr) => {
    const card = Number(curr);
    const reps = repetitions[card];

    if (card === 11) return acc;
    if (reps > maxReps) maxReps = reps;

    return acc + 10 ** (reps - 1)
  }, 0)

  type = type - 10 ** (maxReps - 1) + 10 ** (maxReps + nJokers - 1)
...
function compareArrays(arr1: number[], arr2: number[]) {
  for (let i = 0; i < arr1.length; i++) {
    const firstValue = arr1[i] !== 11 ? arr1[i] : 0;
    const secondValue = arr2[i] !== 11 ? arr2[i] : 0;
